...
import { limiterAuthenticated, limiterUnauthenticated } from "../server.js";
...
export function rateLimitM(req, res, next) {
  const token = req.headers.authorization?.split(" ")[1];

  if (!token) {
    console.log("limiterUnauthenticated()")
    limiterUnauthenticated(req, res, next)
  } else {
    console.log("limiterAuthenticated()")
    limiterAuthenticated(req, res, next)
  }
}